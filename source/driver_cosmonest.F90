...
! This is a driving routine that illustrates the use of the program.
	...
	integer, parameter :: action_nested=3
       ...
       instance = 0 
        ...
        else if (action == action_nested) then
            LogFile = trim(rootname)//'.log'
	   ...
	   print*, new_live
        ...
        Use_growth = Ini_Read_Logical('use_growth',.false.)
        ...
        use_LSS = Use_LSS .or. Use_mpk .or. Use_Clusters .or. Use_WeakLen .or. Use_Lya .or. Use_growth
        ...
        if (action == action_MCMC) then
         ...
         call IO_Close(outfile_handle)
          ...
          if (Feedback > 0) write (*,*) 'starting nested sampling'
          call Nestsample()
          if (Feedback > 0) write (*,*) 'finished'
